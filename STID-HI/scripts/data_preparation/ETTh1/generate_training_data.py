...
def generate_data(args: argparse.Namespace):
    target_channel = args.target_channel #[0]
    future_seq_len = args.future_seq_len #336
    history_seq_len = args.history_seq_len #336
    add_time_of_day = args.tod  #True
    add_day_of_week = args.dow  #True
    add_day_of_month = args.dom  #True
    add_day_of_year = args.doy #True
    output_dir = args.output_dir #datasets/ETTh1
    train_ratio = args.train_ratio #0.6
    valid_ratio = args.valid_ratio#0.2
    data_file_path = args.data_file_path
    steps_per_day = args.steps_per_day #24

    # read data
    df = pd.read_csv(data_file_path) #（17420，8）
    df_index = pd.to_datetime(df["date"].values, format="%Y-%m-%d %H:%M").to_numpy()  # 17420
    df = df[df.columns[1:]] #（17420，7）
    df.index = df_index

    data = np.expand_dims(df.values, axis=-1) #(17420,7,1)

    data = data[..., target_channel]  #(17420,7,1)
    print("raw time series shape: {0}".format(data.shape))

    l, n, f = data.shape #n 7,f 1,l 17420
    num_samples = l - (history_seq_len + future_seq_len) + 1 #16749
    train_num_short = round(num_samples * train_ratio) #10049
    valid_num_short = round(num_samples * valid_ratio) #3350
    test_num_short = num_samples - train_num_short - valid_num_short #3350
    ...
    processed_data = np.concatenate(feature_list, axis=-1)  #(17420,7,5)
