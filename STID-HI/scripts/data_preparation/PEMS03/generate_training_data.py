...
def generate_data(args: argparse.Namespace):
    ...
    future_seq_len = args.future_seq_len #【12】
    history_seq_len = args.history_seq_len#【12】
    add_time_of_day = args.tod #True
    add_day_of_week = args.dow #True
    output_dir = args.output_dir #PEMS
    train_ratio = args.train_ratio #0。6
    valid_ratio = args.valid_ratio# 0.2
    data_file_path = args.data_file_path#PSME
    graph_file_path = args.graph_file_path#PSME
    steps_per_day = args.steps_per_day #288

    # read data
    data = np.load(data_file_path)["data"] #(26208,358,1)
    data = data[..., target_channel]#(26208,358,1)
    print("raw time series shape: {0}".format(data.shape))

    l, n, f = data.shape# n 358 f 1 l 26208
    num_samples = l - (history_seq_len + future_seq_len) + 1 #26185
    train_num_short = round(num_samples * train_ratio) #15711
    valid_num_short = round(num_samples * valid_ratio) #5237
    test_num_short = num_samples - train_num_short - valid_num_short #5237
    ...
    processed_data = np.concatenate(feature_list, axis=-1) #(26208,358,3)
